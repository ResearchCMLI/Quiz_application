...
def session_quiz(userid, username):
    ...
    cursor =  conn.cursor()
    query = """
        INSERT INTO quiz_sessions (user_id, username)
        VALUES (%s, %s)
        """
...
def save_questions(quiz_id, userid, username, questions, answer, correct_or_not, topic, difficulty_level, start_time, end_time, question_no,cosine_similarity):
    conn =  create_connection()
    cursor = conn.cursor()
    query = """
        INSERT INTO quiz_details (quiz_id,id, username, questions, answer,correct_or_not, topic, difficulty_level, start_time, end_time, question_no,cosine_similarity)
...
def insert_time(local_time, userid):
    conn = create_connection()
    cursor = conn.cursor()
    ...
    if result:
        ...
        update_sql = """
        UPDATE login_logs
        SET quiz_started_time = %s
        WHERE session_id = %s
        """
        cursor.execute(update_sql,(local_time,session_id))
        conn.commit()
    else:
...
def fetch_questions(num):
    conn = create_connection()
    cursor = conn.cursor(dictionary=True)
    query = """
        SELECT question_text, topic, difficulty_level,answer
        FROM questions
        ORDER BY RAND()
        LIMIT %s
    """
...
def score_details(user_id,username,quiz_id,quiz_type,score):
    ...
    cursor = conn.cursor()
    query = """
         INSERT INTO score_details (user_id,username,quiz_id,quiz_type,score) VALUES (%s,%s,%s,%s,%s)
         """
...
def quiz_page():
    ...
    if "username" not in st.session_state or "time_limit" not in st.session_state:
        st.error("Quiz not configured properly. Please go back.")
    ...
    if st.session_state.quiz_submitted:
       ...
       if not st.session_state.score_saved:
            score_details(st.session_state.userid,st.session_state.username,st.session_state.quiz_id,st.session_state.quiz_type,total_score)
            st.session_state.score_saved = True
       for i, q in enumerate(st.session_state.questions):
            st.markdown(f"**Q{i+1}:** {q['question_text']}")
            st.write(f"Your answer: {st.session_state.answers[i]}")
            st.write(f"Topic: {q['topic']}, Level: {q['difficulty_level']}")
            st.markdown("---")
    ...
    if "quiz_started" not in st.session_state or not st.session_state.quiz_started:
        ...
        st.session_state.questions = fetch_questions(st.session_state.number_of_questions)
        st.session_state.current_index = 0
        st.session_state.answers = [""] * len(st.session_state.questions)
        if "question_saved_flags" not in st.session_state:
            st.session_state.question_saved_flags = [False] * st.session_state.number_of_questions
        if "next_clicked_flags" not in st.session_state:
            st.session_state.next_clicked_flags = [False] * st.session_state.number_of_questions
        if "correct_flags" not in st.session_state:
            st.session_state.correct_flags = [0] * len(st.session_state.questions)
        st.session_state.question_start_times = [None] * len(st.session_state.questions)
        st.session_state.question_end_times = [None] * len(st.session_state.questions)
        st.session_state.start_time = datetime.now()
        ist = timezone('Asia/Kolkata')
        st.session_state.local_time = st.session_state.start_time.astimezone(ist)
        insert_time(st.session_state.local_time,st.session_state.userid)
...
"""
    ...
    st.markdown(f"""
          ...
          <div style='text-align:center; font-size:18px; margin-top:5px;'>
               Topic: {q['topic']} &nbsp;&nbsp; | &nbsp;&nbsp; Level: {q['difficulty_level']}
          </div>
    ...
    def next_question():
        ...
        if not st.session_state.question_saved_flags[idx]:
              ...
              end_time_utc = st.session_state.question_end_times[idx]
              q = st.session_state.questions[idx]
              #st.write(f"--- Attempting to save question {idx+1} ---")
              #st.write(f"Question Text: {q['question_text']}")
              #st.write(f"User Answer: {ans}")
              #st.write(f"Correct Flag: {int(match)}")
              #st.write(f"Saved Already?: {st.session_state.question_saved_flags[idx]}")
              #st.write(f"Start Time: {start_time_utc}")
              #st.write(f"End Time: {end_time_utc}")
    ...
    if idx == total_questions - 1:
       with col2:
         if st.button("Submit Quiz"):
            ...
            if st.session_state.question_end_times[idx_cur] is None:
