...
def save_questions(quiz_id, userid, username, questions, answer, correct_or_not, difficulty_level, start_time, end_time, question_no,cosine_similarity):
    ...
    cursor = conn.cursor()
    query = """
        INSERT INTO quiz_details_fine (quiz_id,id, username, questions, answer,correct_or_not, difficulty_level, start_time, end_time, question_no, cosine_similarity)
...
def fetch_questions(num):
    ...
    query = """
        SELECT question_text,bloom_level,answer
...
def score_details(user_id,username,quiz_id,quiz_type,score):
    ...
    cursor = conn.cursor()
    query = """
         INSERT INTO score_details_fine (user_id,username,quiz_id,quiz_type,score) VALUES (%s,%s,%s,%s,%s)
         """
...
def quiz_fine():
    ...
    if "quiz_started" not in st.session_state or not st.session_state.quiz_started:
        ...
        if "question_saved_flags" not in st.session_state or len(st.session_state.question_saved_flags) != total_questions:
            st.session_state.question_saved_flags = [False] * st.session_state.number_of_questions
        if "next_clicked_flags" not in st.session_state or len(st.session_state.next_clicked_flags)!= total_questions:
            st.session_state.next_clicked_flags = [False] * st.session_state.number_of_questions
        if "correct_flags" not in st.session_state or len(st.session_state.correct_flags)!= total_questions:
    ...
    def next_question():
        ...
        if not st.session_state.question_saved_flags[idx]:
              ...
              save_questions(
                   ...
                   difficulty_level=q['bloom_level'],
