...
def save_quiz_config(user_id, username, number_of_questions, time_limit):
    conn = create_connection()
    cursor = conn.cursor()

    query = """
    INSERT INTO quiz_configuration (user_id, username, number_of_questions, time_limit)
    VALUES (%s, %s, %s, %s)
    """
...
def quiz_type(userid):
    ...
    if result:
        ...
        WHERE session_id = %s
        """
        cursor.execute(update_sql, ("Not Fine Tuned", session_id))
        conn.commit()
    else:
...
def configure_page():
    st.markdown("<h3 class='material-section-title'>Configure Quiz T5</h3>", unsafe_allow_html=True)
    render_sidebar()
    ...
    if st.button("Start Quiz"):
            # ? Save to DB
            #attempt_id = save_quiz_config(user_id,username, number_of_questions, time_limit)
            save_quiz_config(user_id,username, number_of_questions, time_limit)
            quiz_type(user_id)
            # ?? Store in session state
            st.session_state.username = username
            st.session_state.number_of_questions = number_of_questions
            st.session_state.time_limit = time_limit
            st.session_state.quiz_type = "Not Fine Tuned"

            # ?? Redirect
            st.success("Quiz configuration saved. Redirecting...")
            st.session_state.page = Page.QUIZ
            st.rerun()
